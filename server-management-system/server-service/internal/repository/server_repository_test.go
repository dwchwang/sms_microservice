...
func TestServerRepository_FindAll_FilterByIDNameAndIPv4(t *testing.T) {
	...
	countQuery := `SELECT count(*) FROM "server_schema"."servers" WHERE server_id ILIKE $1 AND server_name ILIKE $2 AND ipv4 LIKE $3 AND "servers"."deleted_at" IS NULL`
	mock.ExpectQuery(regexp.QuoteMeta(countQuery)).
		WithArgs("%SRV-WEB%", "%web%", "10.0.%").
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))

	dataQuery := `SELECT * FROM "server_schema"."servers" WHERE server_id ILIKE $1 AND server_name ILIKE $2 AND ipv4 LIKE $3 AND "servers"."deleted_at" IS NULL ORDER BY server_id ASC LIMIT $4`
...
func TestServerRepository_FindAll_AllFiltersAndPaginationBounds(t *testing.T) {
	...
	countQuery := `SELECT count(*) FROM "server_schema"."servers" WHERE status = $1 AND server_id ILIKE $2 AND server_name ILIKE $3 AND ipv4 LIKE $4 AND os ILIKE $5 AND location ILIKE $6 AND "servers"."deleted_at" IS NULL`
	mock.ExpectQuery(regexp.QuoteMeta(countQuery)).
		WithArgs("on", "%SRV%", "%web%", "10.0.%", "%Ubuntu%", "%HN%").
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))

	dataQuery := `SELECT * FROM "server_schema"."servers" WHERE status = $1 AND server_id ILIKE $2 AND server_name ILIKE $3 AND ipv4 LIKE $4 AND os ILIKE $5 AND location ILIKE $6 AND "servers"."deleted_at" IS NULL ORDER BY updated_at DESC LIMIT $7`
	mock.ExpectQuery(regexp.QuoteMeta(dataQuery)).
...
func TestServerRepository_ExistsByServerNameExclude(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT count(*) FROM "server_schema"."servers" WHERE (server_name = $1 AND server_id != $2) AND "servers"."deleted_at" IS NULL`)).
